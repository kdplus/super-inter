...
def test(normal=1, writer=None, skip_num=1):
    ...
    vimeos_data_list_path = "data_list/vid4_testlist.txt"

    VIMEOS_PATH_BASE_SR = '/home/ubuntu/data/vid4/FRVSR_VID4/HR/'
    VIMEOS_PATH_BASE = '/home/ubuntu/data/vid4/FRVSR_VID4/LR/'
...
#     shuffle(data_list_sr)
    ...
    psnr_a = 0
    psnr_given = 0
    psnr_list = [0 for i in range(0 , unit_size)]
    psnr_middle = 0
    ssim_a = 0
...
#     up_sampling = torch.nn.Upsample(scale_factor=2, mode='bilinear')
    ...
    with torch.no_grad():
        ...
        for step in t:
            ...
            for i in range(2, unit_size + 2):
                batch_data_list_frames.append(list(map(lambda e : e + '/frame_' + "%04d"%(i) + '.png', batch_data_list)))
                batch_data_list_frames_sr.append(list(map(lambda e : e + '/frame_' + "%04d"%(i) + '.png', batch_data_list_sr)))
            ...
            scale_down_x = ori_w / 448
            scale_down_y = ori_h / 256
            print(ori_w, ori_h)
            
            scale_down = (scale_down_y, scale_down_x)
#             scale_down = (1, 1)
...
#             invar = torch.nn.functional.interpolate(torch.nn.functional.interpolate(down_sampling(down_sampling(invar_sr)),
            ...
            if normal == 1:
                ...
                no_middle = True
                pair_index_list = [(i, i+1) for i in range(0, unit_size-1)]
                ind_a = [i for i in range(0, unit_size)]
...
#                 warped_frame0_ft = warp(f05_ret, flow01_enl / 2.0, l0_size, scale_down=(1,1))
                warped_frame0_f1 = warp(f1_enl, flow01_enl, l0_size, scale_down=scale_down)
#                 warped_frame1_ft = warp(f05_ret, flow10_enl / 2.0, l0_size, scale_down=(1,1))
                warped_frame1_f0 = warp(f0_enl, flow10_enl, l0_size, scale_down=scale_down)
...
#                 warped_frame1_ft_4_lr = torch.nn.functional.conv2d(warped_frame1_ft, weight, bias=None, stride=4, padding=0)
                ...
                f0_food = torch.cat((f0_lr, warped_frame0_f1_4_lr, warped_frame0_f1_4_lr), 1)
                f1_food = torch.cat((f1_lr, warped_frame1_f0_4_lr, warped_frame1_f0_4_lr), 1)
...
#                 per_loss += perception_loss(f1_ret, f1_sr)
            ...
            for ind in range(0, unit_size):
                ...
                for img in range(0, batch_size):
                   # if skip_num == 1:
                   #     ## save result picture
                    store_base = './vid4/foliage/'
                    PIL.Image.fromarray((final_output[img].cpu().clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, ::-1] * 255.0).astype(numpy.uint8)).save(store_base+'frame_' + "%04d"%ind + '.png')         
            ...
            ## update progress bar
            cnt += 1
            t.set_description('psnr: %g ssim: %g' % ((psnr_a/(cnt*batch_size*unit_size)), (ssim_a/(cnt*batch_size*unit_size))))
            ...
            if cnt % 50 == 0 or step == 0: 
                ...
                for ind in range(0, unit_size-1):
                    writer.add_image(prefix + '/left_a'+str(ind), show(vutils.make_grid(left_list[ind].clamp(0.0, 1.0).data.cpu(), normalize=False, scale_each=True)), real_step + cnt)
    ...
    final_ssim = ssim_a / (cnt*batch_size*unit_size) 
    final_psnr = psnr_a / (cnt*batch_size*unit_size)
